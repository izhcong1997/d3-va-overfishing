...
var prev;
var next;
const duration = 50; // animation duration between keyframes in milliseconds
...
const getCountries = async (file) => {
  const response = await fetch(file);
  return await response.json();
};

// get all names
const getNames = (data, countries) => {
  var x = data
    .map((d) => d.name)
    .filter((name) => countries.map((c) => c.name).includes(name));
  return new Set(x);
}
...
async function main() {
  ...
  const countries = await getCountries("countries.json");
  names = getNames(data, countries);
